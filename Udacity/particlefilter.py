#particle filter (using robot class)
...
# and then move by 5.
    ...
    x = robot() #create object
    x = set_noise(0.05,0.05,5.0) #for gaussian
    #x = x.move(0.1,5.0)
    p.append(x)
...
w = []
for i in range(N):
    w.append(p[i].measurement_prob(Z)) #Z is the sense data
...
p3 = []
#w contains list of sense data with importance weights
beta = 0.0
...
for t in range(T):
    ...
    print eval(myrobot,p)
#robot position and particle set.
#computes average error in x and y
